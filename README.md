...
执行边界命令安全分类器，拦截 opencode v2 原生 `shell` 工具（v1 的 `bash`），在每条命令**执行前**做静态审查 + 可选的动态 LLM 审查。本目录是 v1 `-next`（0.4.2）到 **opencode v2 promise 插件 API**（`{ id, setup(ctx) }`）的移植版。
...
|---|---|---|
| 函数插件 `{ id, server }` | `export default { id, setup(ctx) }`，setup 返回 cleanup | v2 promise 插件形状 |
...
| `event` hook（`session.deleted`） | `ctx.event.subscribe()` 后台消费循环 | wire 事件 `{ type, data: { sessionID } }`，cleanup 时 `iterator.return()` 停止 |
...
2. **拒绝语义**：`execute.before` 是 v2 唯一可失败的 tool hook。本插件以 `_tag: "Tool.Error"` 形状的错误拒绝调用（源码核实：session runner 仅对 `_tag === "Tool.Error"` 的失败执行 `catchTag` → `failTool`，把消息作为**本次工具调用**的失败返回给模型；普通 `Error` 会导致整个 step 失败）。静态/动态/policy 拒绝消息与 v1 格式一致（含 `DO NOT retry...` 后缀）。
3. **目录解析**：v2 ctx 无 `directory/worktree`，按会话从 `ctx.session.get().location.directory` 解析（带 30 分钟 TTL 与 512 条目上限的缓存），失败回退 `process.cwd()`。
4. **会话清理**：`session.deleted` 事件经 `ctx.event.subscribe()` 消费，事件形状为 `{ type, data: { sessionID } }`。
